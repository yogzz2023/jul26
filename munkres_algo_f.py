...
def hungarian_algorithm(cost_matrix):
    # Convert the cost matrix to a square matrix
    max_cost = np.max(cost_matrix)
    cost_matrix = np.pad(cost_matrix, ((0, max(cost_matrix.shape) - cost_matrix.shape[0]), (0, max(cost_matrix.shape) - cost_matrix.shape[1])), constant_values=max_cost)
    ...
    for i in range(1, n):
        ...
        while marked_i != -1:
            visited[marked_i] = True
            min_j = -1
            delta = np.inf
            for j in range(1, n):
                if not visited[j]:
                    cur = cost_matrix[marked_i, j] - u[marked_i] - v[j]
                    if cur < mins[j]:
                        mins[j] = cur
                        links[j] = marked_index
                    if mins[j] < delta:
                        delta = mins[j]
                        min_j = j
            for j in range(n):
                if visited[j]:
                    ...
                    mins[j] -= delta
            marked_j = min_j
            marked_index = links[marked_j]
            marked_i = p[marked_j]

        while links[marked_j] != -1:
            p[marked_j] = p[links[marked_j]]
            marked_j = links[marked_j]
        p[marked_j] = i

    result = [(p[j], j) for j in range(1, n)]
    return result
...
def data_association(predictions, measurements):
    ...
    indexes = hungarian_algorithm(np.array(cost_matrix))
    return indexes
